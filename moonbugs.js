...
class Level1 extends Phaser.Scene {
    ...
    create() {
        ...
        this.spiralX = Phaser.Math.Between(50, 750);
        this.spiralY = Phaser.Math.Between(50, 470);
        this.spiralGalaxy = this.physics.add.staticSprite(this.spiralX, this.spiralY, 'spiral_galaxy');
        // create star
        this.starX = Phaser.Math.Between(50, 750);
        this.starY = Phaser.Math.Between(50, 470);   
        this.star = this.physics.add.staticSprite(this.starX, this.starY, 'star');
        // create black hole
        this.blackHoleX = Phaser.Math.Between(50, 750);
        this.blackHoleY = Phaser.Math.Between(50, 470);   
        this.blackHole = this.physics.add.staticSprite(this.blackHoleX, this.blackHoleY, 'black_hole');
        // create asteroids
        this.asteroid = this.physics.add.staticGroup();
        for (let i = 0; i < 5; i++) {
            var x = Phaser.Math.Between(0, 800);
            var y = Phaser.Math.Between(0, 600);
            this.asteroid.create(x, y, 'asteroid');
        }

        while (this.bugX == 0 && this.bugY == 0 || Math.abs(this.bugX - this.starX) < 50 && Math.abs(this.bugY - this.starY) < 50
        || Math.abs(this.bugX - this.spiralX) < 50 && Math.abs(this.bugY - this.spiralY) < 50
        || Math.abs(this.bugX - this.blackHoleX) < 50 && Math.abs(this.bugY - this.blackHoleY) < 50) {
            this.bugX = Phaser.Math.Between(20, 780);
            this.bugY = Phaser.Math.Between(20, 580);       
        }

        this.bug = this.physics.add.sprite(this.bugX, this.bugY, 'bug', 0); // create bug before cannon so it's hidden under cannon
    ...
    update() {
        ...
        if (this.bug.body.blocked.down && this.bug.body.velocity.x === 0 && this.canCollect == true) {
            if (this.attempt < 3) {
                ...
                while (Math.abs(x - this.starX) < 50 && Math.abs(y - this.starY) < 50
                || Math.abs(this.x - this.spiralX) < 50 && Math.abs(this.y - this.spiralY) < 50
                || Math.abs(this.x - this.blackHoleX) < 50 && Math.abs(this.y - this.blackHoleY) < 50) {
