        ...
        if (this.bug.body.blocked.down && this.bug.body.velocity.x === 0 && this.canCollect == true) {
            if (this.score == 25) {
                ...
                this.time.delayedCall(4000, () => {
                    this.scene.start('Level6', { bugColor: this.bugColor, cumulativeScore: this.score + this.runningTotal, levels: this.levels + 1 }); // start next level after delay
                });
                ...
                if (this.score >= this.needed) { // check for winning level
                    ...
                    this.time.delayedCall(750, () => {
                        if (!this.transitionToCheckScore) {
                            console.log('level 5 to CheckScore');
                            this.transitionToCheckScore = true;
                            this.scene.start('CheckScore', { bugColor: this.bugColor, cumulativeScore: this.score + this.runningTotal, level: 5 }); // start next level after delay
                        }
                    });
                }
            }
