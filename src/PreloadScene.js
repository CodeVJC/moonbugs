class PreloadScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.h3Explainer = this.add.text(120, 290, `= Collect H-3 to win! Maintain an average of 20 per level.`, { fontFamily: 'Arial', fontSize: '24px', fill: '#00ffff' });
        this.star = this.physics.add.staticSprite(100, 340, 'star');
        this.starExplainer = this.add.text(120, 330, '= Star, +1 attempts', { fontFamily: 'Arial', fontSize: '24px', fill: '#00ffff' });
        this.spiralGalaxy = this.physics.add.staticSprite(130, 380, 'spiral_galaxy');
        this.spiralExplainer = this.add.text(180, 370, '= Spiral galaxy, -1 attempts', { fontFamily: 'Arial', fontSize: '24px', fill: '#00ffff' });
        this.blackHole = this.physics.add.staticSprite(130, 430, 'black_hole');
        this.blackHoleExplainer = this.add.text(180, 420, '= Black hole! Game Over', { fontFamily: 'Arial', fontSize: '24px', fill: '#00ffff' });
        this.satellite1 = this.physics.add.staticSprite(155, 480, 'satellite', 0);
        this.satellite1Explainer = this.add.text(220, 470, `= Satellite, rebound in direction of arrow`, { fontFamily: 'Arial', fontSize: '24px', fill: '#00ffff' });
        this.welcomeText = this.add.text(180, 150, 'MOONBUGS', { fontFamily: 'Arial', fontSize: '70px', fill: '#00ffff'});
